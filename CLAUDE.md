...
assets/                 Identidade visual (ícone dos apps e a arte de origem)
...
diagnostico-terminal.bat  Coleta o estado do auto-update de uma tela (rodar no mini-PC)
```

**Identidade visual.** `assets/cpe.ico` é o que vale em execução: os dois `.csproj` o
declaram em `<ApplicationIcon>` e os dois `.iss` em `SetupIconFile`. Ele está em formato
**BMP multi-tamanho (16 a 256)** — um `.ico` montado a partir de PNG **não funciona** no
GDI nem no Inno Setup, e o ícone simplesmente não aparece. `cpe-logo.png` (128px) e
`cpe-logo-512.png` (512px) são a arte de origem, não usadas em execução: servem para
regerar o ícone e para material de apresentação.
...
a rotação e o layout principal, que guarda a tela dona nas configurações. Numa parede com
vários terminais, misturar layouts significa projetar o conteúdo errado na TV errada.
...
Versão **1.56.0**. Fases A (parede local) e D (rede, controle remoto, auto-update)
